    ...
    <div class="container my-5 fontePrincipal col-12 col-lg-10">
        <div class="row my-3">
            ...
            <div class="col-1 offset-11">
                <a href="{{ route('forum.list', $post->categoria_id) }}">
                    <button class="botaoPrincipal rounded-pill px-3">
                        Voltar
                    </button>
                </a>
            </div>
        ...
        <div class="row">
            <div class="secaoPost col-12 px-5">
                <h3 class="fonteBold mb-5"> {{ $post->titulo }} </h3>
                <p> {{ $post->texto }} </p>
            ...
            <div class="row mt-3">
                ...
                <div class="col">
                    ...
                    @if (Auth::check() && Auth::user()->id == $post->user_id)
                        <a href="{{ route('forum.edit', $post->id) }}"><button
                                class="btn btn-warning rounded-pill px-3">Editar</button></a>
                    @endif
        ...
        <div class="secaoComentarios">
                            ...
                            <div class="row">
                                <div class="col-4 justify-self-end mt-3">
                                    <form action="{{ route('forum.delete', $comment->id) }}" method="POST">
                                        @method('DELETE')
                                        @csrf
                                        <p><button type="submit" class="btn btn-danger rounded-pill px-3">Excluir
                                                comentário</button></p>
                                    </form>
                                </div>
                            </div>
            ...
            <div class="comentarioForm">
                <form action="{{ route('forum.comment') }}" method="POST">
                    ...
                    <textarea class="form-control commentInput" name="texto" rows="3" placeholder="Seu comentário aqui"></textarea>
